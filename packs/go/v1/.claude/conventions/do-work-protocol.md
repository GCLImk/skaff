...
├── scout/                          go-scout findings briefs
...
| `do-work/scout/`             | No        | No                  | go-scout only         |
...
| Writing the `## Plan` section into the REQ file        | go-implement (plan-only mode) |
| Writing the `## Plan Hash` section into the REQ file   | go-implement agent |
...
| Writing code under first-party Go packages and `*_test.go` files                 | go-implement agent |
| Godoc comment and markdown edits                             | go-doc-writer agent|
| Scout findings under `do-work/scout/`                  | go-scout agent     |
...
- Agents must not edit the `## Plan Hash` section. It is owned by go-implement and is the drift-detection anchor.
