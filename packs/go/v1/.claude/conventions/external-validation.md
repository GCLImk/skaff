...
5. Documentation accuracy - do the godoc comments and markdown updates accurately describe
...
- Do not chain an external validation onto an external validation. The second reviewer is final. If it fails, the change loops back to `go-implement`, not to a third reviewer.
