...
| --------------- | ----------------------------------------------------------------------- |
| `parse_check`   | Every package builds without syntax or type errors. Binary signal.      |
| `lint`          | Lint and analyser violations relative to changed lines and file count.  |
| `complexity`    | Cyclomatic and cognitive complexity distribution across modified funcs.  |
| `structure`     | Module layout, package boundaries, import hygiene, and DI consistency.  |
| `dead_code`     | Unreachable code, unused symbols, orphaned files introduced by change.  |
| `test_coverage` | Line coverage across modified packages.                                 |
| `doc_quality`   | Godoc completeness on exported types and functions; markdown integrity. |

Each dimension is **N/A-aware** - when a dimension genuinely cannot be scored (for example no test package exists or no analyser is configured), the score is recorded as `null` and dropped from the composite weighted average rather than penalised as zero.
...
`parse_check` is a gate, not a weight. If `parse_check < 1.0`, the composite is 0 regardless of other scores. No lint score can rescue code that does not build.
...
- No tooling exists to measure it (for example no test package exists -> `test_coverage = null`).
...
A dimension appears when it was `null` on the previous baseline and has a real value on the current run. This happens when tooling is added mid-project - for example REQ-003 adds a test suite and `test_coverage` goes from `null` to `0.12`.
...
1. **Graduated kept bar.** Higher scores demand broader improvement. A single comment tweak cannot graduate a feature from 0.87 to 0.875.
...
If `Kept: No`, the ratchet agent returns a blocking verdict to the main session and the REQ loops back to `go-implement` with the failing dimensions as the task brief. The archive step does not run until the ratchet keeps.
