---
name: do-work-run
description: Drain the do-work queue. Main session orchestrates; spawns go-scout, go-implement, go-doc-writer, reviewer, ratchet, and git-workflow specialists per REQ. Use when the user says "do work run", "work", "go", "start", or confirms the work prompt.
allowed-tools:
  - Read
  - Write
  - Edit
...
| ------------------------------------------------------- | --------------------------------- |
| `## Plan` or `## Plan Hash` content                     | `go-implement` in plan-only mode  |
| Any `.go` file, `go.mod`, or `go.sum` edit              | `go-implement` in implement mode  |
| Godoc comment edits, README updates, doc audit tables   | `go-doc-writer`                   |
...
| Scout findings briefs                                   | `go-scout`                        |
...
2. **Architectural decisions** - a plan or implementation proposes a cross-REQ or cross-surface change not explicitly authorised in the REQ (new table, new integration, new dependency, new service boundary). Do not let a specialist silently expand scope; surface it.
...
4. **Scout** (medium and complex only): spawn `go-scout` via `Agent(subagent_type: "go-scout", prompt: <dispatch brief>)`. Wait for its findings brief at `do-work/scout/REQ-NNN-*-findings.md`.
5. **Plan**: spawn `go-implement` in **plan-only mode**. Brief must name the mode explicitly. The agent writes `## Plan` and `## Plan Hash` sections inline in the REQ file and returns without touching code. Plan depth scales to complexity (Simple: 1-3 bullets; Medium: ordered steps with verification checks; Complex: ordered steps, dependencies, testing approach, rollback notes).
...
7. **Implement**: spawn `go-implement` with the full REQ content (including `## Plan`) in **implement mode**.
8. **Documentation**: if complex, or if docs are explicitly in scope, spawn `go-doc-writer`.
9. **Review**: spawn `reviewer`. Act on verdict:
   - `Approve` - proceed to step 10.
   - `Request Changes` - increment the REQ's `implement_review_ratchet_cycles` counter (Loop Counters section), write back. If counter > 5, escalate. Otherwise re-spawn `go-implement` with blocking issues as the task brief. Return to step 9.
...
10. **Ratchet**: spawn `ratchet`. Act on verdict:
    ...
    - `Rejected` - increment the loop counter. If > 5, escalate. Otherwise re-spawn `go-implement` with failing dimensions from Blocking Dimensions. Return to step 9 (reviewer re-reviews before ratchet re-runs).
...
- Same blocking issue returned to `go-implement` more than twice.
