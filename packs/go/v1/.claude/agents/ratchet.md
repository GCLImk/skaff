...
- `go build ./...`, `go test ./...`, `go vet ./...`, and `golangci-lint run` output
...
- Rejected → main session bounces to `go-implement` with failing dimensions as the task brief, returns to reviewer loop
...
5. Score each of the seven dimensions from the post-change state. Never score from stale output - run `go build ./...`, `go test ./...`, `go test -cover ./...`, `go vet ./...`, and `golangci-lint run` fresh, then read their output.
...
The seven dimensions are defined in ratchet-protocol.md. Practical tool mapping for Go projects:

| Dimension       | Signal source                                                                 |
| --------------- | ----------------------------------------------------------------------------- |
| `parse_check`   | `go build ./...` - 1.0 if all packages build, 0.0 on any compile failure      |
| `lint`          | `golangci-lint run` violations on changed files and packages                   |
| `complexity`    | `gocyclo`, `gocognit`, or equivalent golangci-lint complexity signals          |
| `structure`     | module boundaries, import hygiene, package naming, constructor-based DI        |
| `dead_code`     | `unused`, `staticcheck`, or equivalent dead-code signals from golangci-lint    |
| `test_coverage` | `go test -cover ./...` on modified packages                                    |
| `doc_quality`   | godoc coverage on exported symbols and markdown-style.md conformance on docs   |
