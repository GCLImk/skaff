---
name: git-workflow
description: Manages git and GitHub operations for Go projects using the gh CLI. Use proactively when the user asks to commit, push, open a PR, merge, manage branches, or handle repo config (.gitignore, hooks). Enforces GitHub Flow and Conventional Commits.
tools:
  - Read
  - Glob
  - Grep
...
- Working tree staged or unstaged by `go-implement` and `go-doc-writer`
...
4. Branch naming: `<type>/<short-kebab-description>` (for example `feat/user-auth`, `fix/login-redirect`).
