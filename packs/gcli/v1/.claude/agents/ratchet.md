...
tools:
  ...
  - "Bash(ruff format*)"
  - "Bash(pytest*)"
  - "Bash(python -c \"import sys; sys.path.insert(0,'cli'); from tools import dispatch; print('OK')\"*)"
  - "Bash(jq*)"
  - "Bash(cat*)"
  - "Bash(tail*)"
  - "Bash(wc*)"
...
- `python -m py_compile`, `ruff check`, `pytest`, and the smoke import output
...
- Rejected → main session bounces to `gcli-implement` with failing dimensions as the task brief, returns to reviewer loop
...
5. Score each of the seven dimensions from the post-change state. Never score from stale build output - run `python -m py_compile` on each modified `.py` file, the dispatcher smoke import, `ruff check` on changed files, and `pytest` fresh, then read their output.
...
The seven dimensions are defined in ratchet-protocol.md. Practical tool mapping for gcli-architecture projects:

| Dimension        | Signal source                                                                                             |
| ---------------- | --------------------------------------------------------------------------------------------------------- |
| `parse_check`    | `python -m py_compile` on every modified `.py` file                                                        |
| `lint`           | `ruff check` on changed files plus `ruff format --check`                                                   |
| `complexity`     | `ruff` McCabe complexity rules (`C901`) on modified functions                                              |
| `structure`      | `_resolve_safe` usage on new tools; `extension/manifest.json` conformance; `from __future__ import annotations` presence |
| `dead_code`      | `ruff` rules `F401`/`F841` on changed files                                                                |
| `test_coverage`  | `pytest` pass/fail plus dispatcher smoke import. Line-level coverage is not enforced; integration smoke is the gate |
| `doc_quality`    | Google-style docstrings on public functions; README freshness for changed modules                          |

If a project lacks a pytest suite, record `test_coverage` as `null` only when the smoke import is the sole available signal and flag it in the initial baseline commentary so the disappearance check does not fire on future runs.
