...
Blocks PRs on objective regressions: type-check failure, lint failure, format drift, test failure, build failure, leaked secrets, or a baseline composite below the project floor.
...
- **Node version** - edit `node-version` in the setup step if you are not on 20.x.
- **Package manager** - swap the `pnpm/action-setup` step for `npm ci` or `yarn install --immutable` if the project does not use pnpm. Update every command from `pnpm <x>` to the chosen manager.
...
- **Type errors** - run `pnpm tsc --noEmit` locally to reproduce. The CI job uses the same command.
- **Lint failures** - run `pnpm lint` locally; the CI job uses the project's `next/core-web-vitals` + `@typescript-eslint/strict-type-checked` config.
