...
- `pnpm tsc --noEmit`, `pnpm lint`, `pnpm test --run --coverage`, and `pnpm build` output
...
- Rejected → main session bounces to `nextjs-implement` with failing dimensions as the task brief, returns to reviewer loop
...
5. Score each of the seven dimensions from the post-change state. Never score from stale output - run `pnpm tsc --noEmit`, `pnpm lint`, `pnpm test --run --coverage`, and `pnpm build` fresh, then read their output.
...
The seven dimensions are defined in ratchet-protocol.md. Practical tool mapping for Next.js + TypeScript projects:

| Dimension        | Signal source                                                                                              |
| ---------------- | ---------------------------------------------------------------------------------------------------------- |
| `parse_check`    | `pnpm tsc --noEmit` AND `pnpm build` - both exit 0 means 1.0; any type error or build failure means 0.0    |
| `lint`           | `pnpm lint` (eslint with `next/core-web-vitals` + `@typescript-eslint/strict-type-checked`) + `pnpm format:check`. Score = 1 - (errors_introduced / errors_baseline_or_files_changed) |
| `complexity`     | `npx eslint --rule '{"complexity":["error",10]}'` distribution on modified files. Score from violation count vs lines changed. |
| `structure`      | App Router + server-only boundary conformance to nextjs-style.md. Penalise any `googleapis`, `next-auth/server`, or `NOTIFY_ENDPOINT` import reachable from a `"use client"` module. |
| `dead_code`      | `npx ts-prune` or `eslint @typescript-eslint/no-unused-vars` warnings introduced by the change             |
| `test_coverage`  | `pnpm test --run --coverage` (vitest V8 provider). Lines coverage on changed files.                        |
| `doc_quality`    | `eslint-plugin-tsdoc` (or `jsdoc/require-jsdoc`) warnings on changed `lib/`, `types/`, `route.ts` + markdown-style.md conformance on any changed docs |

If the project lacks a tool for a dimension (e.g. no `ts-prune` installed, no tsdoc lint rule wired in), record the dimension as `null` and flag it in the initial baseline commentary so the disappearance check does not fire on future runs.
