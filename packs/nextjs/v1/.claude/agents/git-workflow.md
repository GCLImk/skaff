---
name: git-workflow
description: Manages git operations and GitHub PR lifecycle for Next.js + TypeScript projects using the gh CLI. Use proactively when the user asks to commit, push, open a PR, merge, manage branches, or handle repo config (.gitignore, hooks). Enforces GitHub Flow and Conventional Commits.
tools:
  - Read
  - Glob
  - Grep
...
- Working tree staged or unstaged by `nextjs-implement` and `nextjs-doc-writer`
