...
If `Kept: No`, the ratchet agent returns a blocking verdict to the main session and the REQ loops back to `nextjs-implement` with the failing dimensions as the task brief. The archive step does not run until the ratchet keeps.
