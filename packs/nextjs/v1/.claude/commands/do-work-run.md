---
name: do-work-run
description: Drain the do-work queue. Main session orchestrates; spawns nextjs-scout, nextjs-implement, nextjs-doc-writer, reviewer, ratchet, and git-workflow specialists per REQ. Use when the user says "do work run", "work", "go", "start", or confirms the work prompt.
allowed-tools:
  - Read
  - Write
  - Edit
...
| --------------------------------------------------------- | ---------------------------------------------- |
| `## Plan` or `## Plan Hash` content                       | `nextjs-implement` in plan-only mode         |
| Any `.ts` / `.tsx` / config file under `app/`, `components/`, `lib/`, `types/`, `tests/`, or repo root | `nextjs-implement` in implement mode |
| TSDoc edits, README updates, doc audit tables             | `nextjs-doc-writer`                            |
...
| Scout findings briefs (project shape, dependencies, integration map) | `nextjs-scout`                       |
| Routing / server-client boundary advice                   | `nextjs-routing-specialist`                    |
| Google Sheets API v4 design advice                        | `sheets-specialist`                            |
| NextAuth + Google OAuth design advice                     | `auth-specialist`                              |
| Dockerfile / Cloud Run / Secret Manager advice            | `cloudrun-specialist`                          |
| ADR drafts (`docs/decisions/NNNN-*.md`)                   | `nextjs-doc-writer`                            |
| Proposed-convention entries (`do-work/proposed-conventions/*.md`) | the relevant domain advisor (routing/sheets/auth/cloudrun) |
| Convention edits in `.claude/conventions/`                | `nextjs-implement` via a normal REQ titled `convention: promote <title>` |
...
1. **Clarification** - the REQ body, scout findings, or a specialist's output contains an Open Question, a "resolve before implementation" note, or a decision that has no defensible default. Examples: a missing JIRA project key, a sheet name not in the existing schema, an ENUM whose values the user has not enumerated.
2. **Architectural decisions** - a plan or implementation proposes a cross-REQ or cross-surface change not explicitly authorised in the REQ (new table, new integration, new dependency, new tier of AppSheet licensing). Do not let a specialist silently expand scope; surface it.
...
- `.claude/conventions/knowledge-protocol.md` (so you know how to surface ADRs and proposed conventions in the loop summary)
...
3. **Triage complexity** by reading the REQ file:
   ...
   - Otherwise classify: **Simple** (config/small fix, location known), **Medium** (clear goal, location unknown), **Complex** (new feature, architectural, multi-system).
4. **Scout** (medium and complex only): spawn `nextjs-scout` via `Agent(subagent_type: "nextjs-scout", prompt: <dispatch brief>)`. Wait for its findings brief at `do-work/scout/REQ-NNN-*-findings.md`.
4a. **Domain advice** (medium and complex only, when the REQ touches a domain): spawn one or more domain specialists in parallel. Each writes its own brief to `do-work/scout/REQ-NNN-<domain>-advice.md`. Pass every advice file path to `nextjs-implement` in step 5 alongside the scout brief.
   - Routing / server-client boundary / server actions: `nextjs-routing-specialist`
   - Google Sheets API v4 reads or writes: `sheets-specialist`
   - NextAuth, Google OAuth, `hd` enforcement, route protection: `auth-specialist`
   - Dockerfile, Cloud Run config, Secret Manager, deploy: `cloudrun-specialist`
   Skip a specialist when the REQ does not touch its domain. All four are read-only advisors - they never write production code.
5. **Plan**: spawn `nextjs-implement` in **plan-only mode**. Brief must name the mode explicitly. The agent writes `## Plan` and `## Plan Hash` sections inline in the REQ file and returns without touching code. Plan depth scales to complexity (Simple: 1-3 bullets; Medium: ordered steps with verification checks; Complex: ordered steps, dependencies, testing approach, rollback notes).
...
7. **Implement**: spawn `nextjs-implement` with the full REQ content (including `## Plan`) in **implement mode**.
8. **Documentation**: if complex, or if docs are explicitly in scope, spawn `nextjs-doc-writer`.
9. **Review**: spawn `reviewer`. Act on verdict:
   - `Approve` - proceed to step 10.
   - `Request Changes` - increment the REQ's `implement_review_ratchet_cycles` counter (Loop Counters section), write back. If counter > 5, escalate. Otherwise re-spawn `nextjs-implement` with blocking issues as the task brief. Return to step 9.
...
10. **Ratchet**: spawn `ratchet`. Act on verdict:
    ...
    - `Rejected` - increment the loop counter. If > 5, escalate. Otherwise re-spawn `nextjs-implement` with failing dimensions from Blocking Dimensions. Return to step 9 (reviewer re-reviews before ratchet re-runs).
...
12a. **Collect knowledge artefacts**: scan every specialist's return summary from this REQ for a `Knowledge Artefacts:` section (per knowledge-protocol.md). Append every listed file (new ADRs, new or bumped proposed-convention entries) to a running list in main-session memory. The orchestrator does NOT read or rewrite the artefact files - only their paths. Mature proposals (Maturity 3) get noted with a `(ready for promotion)` tag. If a specialist returned without a `Knowledge Artefacts:` section at all, that is a protocol violation - log it as a warning and continue (do not block the REQ).
...
- Same blocking issue returned to `nextjs-implement` more than twice.
...
- [ ] Loop summary contains a top-level `## Knowledge Artefacts This Run` section listing every ADR added or superseded and every proposed-convention entry created or bumped, with each Maturity-3 entry tagged `(ready for promotion)`. If no artefacts landed, write the heading with `_None this run._` underneath rather than omit it
