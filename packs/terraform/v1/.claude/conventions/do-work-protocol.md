...
├── scout/                          tf-scout findings briefs
...
| `do-work/scout/`             | No        | No                  | tf-scout only         |
...
| Writing the `## Plan` section into the REQ file        | tf-implement (plan-only mode) |
| Writing the `## Plan Hash` section into the REQ file   | tf-implement agent |
...
| Writing Terraform under `*.tf` and `*.tftest.hcl`     | tf-implement agent |
| Terraform descriptions and markdown edits             | tf-doc-writer agent |
| Scout findings under `do-work/scout/`                 | tf-scout agent     |
...
- Agents must not edit the `## Plan Hash` section. It is owned by tf-implement and is the drift-detection anchor.
