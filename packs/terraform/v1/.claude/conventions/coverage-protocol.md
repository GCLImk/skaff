...
## Terraform Test Coverage Note

In Terraform projects, the ratchet's `test_coverage` dimension means module and resource coverage proven by `terraform test`.

- The pack default threshold is `0.50`.
- Every touched module should have at least one `.tftest.hcl` file.
- Plan-only assertions are the minimum acceptable floor. Add apply assertions only when the repository already supports them.
- A module with no `terraform test` coverage should be treated as uncovered, not N/A, unless the REQ explicitly says the module is documentation-only.
