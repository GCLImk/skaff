...
5. Documentation accuracy - do variable descriptions, output descriptions,
   README sections, and comments accurately describe the infrastructure, or are
   they stale?
...
- Do not chain an external validation onto an external validation. The second reviewer is final. If it fails, the change loops back to `tf-implement`, not to a third reviewer.
