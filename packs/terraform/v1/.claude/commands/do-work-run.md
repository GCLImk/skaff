---
name: do-work-run
description: Drain the do-work queue. Main session orchestrates; spawns tf-scout, tf-implement, tf-doc-writer, reviewer, ratchet, and git-workflow specialists per REQ. Use when the user says "do work run", "work", "go", "start", or confirms the work prompt.
allowed-tools:
  - Read
  - Write
  - Edit
...
**You (the main Claude Code session) are the orchestrator.** Read this command body, then drain the queue. Delegate all implementation, doc, review, ratchet, and git work to specialist sub-agents via the Agent tool. You own state transitions and escalation decisions only. You do not write infrastructure code, docs, or commits yourself.
...
| ------------------------------------------------------------- | ---------------------------- |
| `## Plan` or `## Plan Hash` content                           | `tf-implement` in plan-only mode |
| Any `.tf`, `.tftest.hcl`, or `.tfvars.example` file           | `tf-implement` in implement mode |
| Variable or output descriptions, module README updates        | `tf-doc-writer` |
...
| Scout findings briefs                                         | `tf-scout` |
...
2. **Architectural decisions** - a plan or implementation proposes a cross-REQ or cross-surface change not explicitly authorised in the REQ, such as a new backend pattern, new provider family, or new secret-management dependency.
...
3. **Triage complexity** by reading the REQ file:
   ...
   - Otherwise classify: **Simple** (small Terraform fix, location known), **Medium** (clear goal, module or provider mapping needed), **Complex** (new module, backend work, or multi-stack change).
4. **Scout** (medium and complex only): spawn `tf-scout` via `Agent(subagent_type: "tf-scout", prompt: <dispatch brief>)`. Wait for its findings brief at `do-work/scout/REQ-NNN-*-findings.md`.
5. **Plan**: spawn `tf-implement` in **plan-only mode**. Brief must name the mode explicitly. The agent writes `## Plan` and `## Plan Hash` sections inline in the REQ file and returns without touching code. Plan depth scales to complexity.
...
7. **Implement**: spawn `tf-implement` with the full REQ content (including `## Plan`) in **implement mode**.
8. **Documentation**: if docs are explicitly in scope, or if variables, outputs, or module READMEs changed, spawn `tf-doc-writer`.
9. **Review**: spawn `reviewer`. Act on verdict:
   - `Approve` - proceed to step 10.
   - `Request Changes` - increment the REQ's `implement_review_ratchet_cycles` counter (Loop Counters section), write back. If counter > 5, escalate. Otherwise re-spawn `tf-implement` with blocking issues as the task brief. Return to step 9.
...
10. **Ratchet**: spawn `ratchet`. Act on verdict:
    ...
    - `Rejected` - increment the loop counter. If > 5, escalate. Otherwise re-spawn `tf-implement` with failing dimensions from Blocking Dimensions. Return to step 9 (reviewer re-reviews before ratchet re-runs).
11. **Git**: spawn `git-workflow`.
12. **Archive**: move REQ file to `do-work/archive/`. Rename to `REQ-NNN-done.md`.
...
- Same blocking issue returned to `tf-implement` more than twice.
...
- Never implement, review, or commit Terraform yourself. Spawn a specialist.
