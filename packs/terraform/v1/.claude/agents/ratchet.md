---
name: ratchet
description: Quality ratchet. Measures the codebase across eight Terraform-specific dimensions after reviewer returns Approve, applies the graduated kept bar, runs external validation at high scores, and either Keeps the change (forwards to git-workflow) or Rejects it (bounces back to tf-implement with failing dimensions as the task brief). Enforces baseline non-regression. See .claude/conventions/ratchet-protocol.md.
tools:
  - Read
  - Grep
  - Glob
...
- Rejected -> main session bounces to `tf-implement` with failing dimensions as the task brief, returns to reviewer loop
...
5. Score each of the eight dimensions from the post-change state. Never score from stale output - run `terraform fmt -check -recursive`, `terraform validate`, `tflint --recursive`, `terraform test`, and `checkov -d . --compact --quiet` fresh, then read their output.
...
The eight dimensions are defined in ratchet-protocol.md. Practical tool mapping for Terraform projects:

| Dimension       | Signal source |
| --------------- | ------------- |
| `parse_check`   | `terraform validate` - exit 0 means 1.0; any validation or parse error means 0.0 |
| `lint`          | `terraform fmt -check -recursive` plus `tflint --recursive` on changed modules |
| `complexity`    | Module fan-out, nested conditionals, `dynamic` blocks, and locals indirection on modified files |
| `structure`     | `terraform-style.md` conformance: file layout, version pinning, provider placement, and module boundaries |
| `dead_code`     | Unused variables, outputs, providers, modules, and orphaned test files introduced by the change |
| `test_coverage` | `terraform test` coverage across touched modules and critical resources |
| `doc_quality`   | Variable and output descriptions plus module README accuracy and markdown-style.md conformance |
| `security`      | `checkov` output plus security-style.md rules around secrets, encryption, public exposure, and IAM |
