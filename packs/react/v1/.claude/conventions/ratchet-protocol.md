...
| `complexity` | Component and hook size, nested render logic, and effect complexity across modified files. |
| `structure` | Project layout, naming, component placement, and hook usage consistency. |
| `dead_code` | Unused exports, unreachable branches, and orphaned files introduced by the change. |
| `test_coverage` | Line and branch coverage across modified components, hooks, and utilities. |
...
If `Kept: No`, the ratchet agent returns a blocking verdict to the main session and the REQ loops back to `react-implement` with the failing dimensions as the task brief. The archive step does not run until the ratchet keeps.
