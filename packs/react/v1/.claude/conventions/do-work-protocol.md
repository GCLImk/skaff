...
├── scout/                          react-scout findings briefs
...
| `do-work/scout/`             | No        | No                  | react-scout only         |
...
| Writing the `## Plan` section into the REQ file        | react-implement (plan-only mode) |
| Writing the `## Plan Hash` section into the REQ file   | react-implement agent |
...
| Writing code under `src/` and `tests/`                 | react-implement agent |
| XML doc and markdown edits                             | react-doc-writer agent|
| Scout findings under `do-work/scout/`                  | react-scout agent     |
...
- Agents must not edit the `## Plan Hash` section. It is owned by react-implement and is the drift-detection anchor.
