---
name: do-work-run
description: Drain the do-work queue. Main session orchestrates; spawns react-scout, react-implement, react-doc-writer, reviewer, ratchet, and git-workflow specialists per REQ. Use when the user says "do work run", "work", "go", "start", or confirms the work prompt.
allowed-tools:
  - Read
  - Write
  - Edit
...
| -------------------------------- | ---------------------------- |
| `## Plan` or `## Plan Hash` content | `react-implement` in plan-only mode |
| Any `.ts`, `.tsx`, `.js`, or `.jsx` file under `src/` or `tests/` | `react-implement` in implement mode |
| JSDoc edits, README updates, or Storybook stories | `react-doc-writer` |
...
| Scout findings briefs | `react-scout` |
...
4. **Scout** (medium and complex only): spawn `react-scout` via `Agent(subagent_type: "react-scout", prompt: <dispatch brief>)`. Wait for its findings brief at `do-work/scout/REQ-NNN-*-findings.md`.
5. **Plan**: spawn `react-implement` in **plan-only mode**. Brief must name the mode explicitly. The agent writes `## Plan` and `## Plan Hash` sections inline in the REQ file and returns without touching code.
...
7. **Implement**: spawn `react-implement` with the full REQ content (including `## Plan`) in **implement mode**.
8. **Documentation**: if complex, or if docs are explicitly in scope, spawn `react-doc-writer`.
9. **Review**: spawn `reviewer`. Act on verdict:
   - `Approve` - proceed to step 10.
   - `Request Changes` - increment the REQ's `implement_review_ratchet_cycles` counter (Loop Counters section), write back. If counter > 5, escalate. Otherwise re-spawn `react-implement` with blocking issues as the task brief. Return to step 9.
...
10. **Ratchet**: spawn `ratchet`. Act on verdict:
    ...
    - `Rejected` - increment the loop counter. If > 5, escalate. Otherwise re-spawn `react-implement` with failing dimensions from Blocking Dimensions. Return to step 9 (reviewer re-reviews before ratchet re-runs).
...
- Same blocking issue returned to `react-implement` more than twice.
