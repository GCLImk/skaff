...
- Register a provider in exactly one module and export it when another module consumes it. Never import across a feature boundary by deep relative path.
- Constructor injection only. Default singleton scope. No `forwardRef()` to paper over a cycle.
- Controllers validate, delegate to one service method, and shape the response. Never return a persistence entity from a controller.
