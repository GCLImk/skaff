...
> **Note on "orchestrator":** Throughout this document "orchestrator" refers to the **main Claude Code session** executing the `/do-work-run` slash command (`.claude/commands/do-work-run.md`). There is no orchestrator sub-agent. A sub-agent cannot spawn sibling sub-agents via the Agent tool, so orchestration runs in the main session and spawns specialists (scout, implement, doc-writer, reviewer, ratchet, git-workflow) directly. "Orchestrator-owned" file sections and state transitions are owned by the main session while that command is running.
...
├── scout/                          nestjs-scout findings and advisor briefs
│   └── REQ-NNN-<topic>-findings.md
├── ratchet/                        quality baselines - append-only
│   └── baselines.jsonl
├── summaries/                      loop, audit, and implementation summaries
├── proposed-conventions/           advisor pattern proposals - see knowledge-protocol.md
│   └── <kebab-title>.md
...
| In progress   | `-in-progress.md`     | `do-work/working/`  | main session (per /do-work-run command) |
| Done          | `-done.md`            | `do-work/archive/`  | main session (per /do-work-run command) |

State transitions are `mv` operations. Only the main session changes state once processing begins.

## Write Permissions by Action

| Location                     | do action | main session (per /do-work-run command) | Any other agent           |
| ---------------------------- | --------- | ------------------- | ------------------------- |
...
| `do-work/scout/`             | No        | No                  | nestjs-scout and the read-only domain advisors only |
| `do-work/ratchet/`           | No        | Read only           | ratchet only (append-only)|
| `do-work/summaries/`         | No        | Yes                 | Any agent may append      |
| `do-work/proposed-conventions/` | No     | Read only           | Domain advisors only (see knowledge-protocol.md) |

## Immutability Rule

Files in `do-work/working/` and `do-work/archive/` are **immutable** to every agent except the main session that owns the transition. No appends, no edits, no "one more thing" patches.
...
Only one main session may run the queue at a time. Concurrent `do work run` invocations against the same repo corrupt state - two main sessions both claim the same REQ, both append scoresets to `baselines.jsonl`, and one produces git output against a stale base.
...
| `complexity`   | `simple` \| `medium` \| `complex`. If present, the main session uses this value directly and skips its own triage pass. Absent means the main session triages from the REQ body. |

The main session appends `claimed_at`, `route`, and `completed_at` during processing. Do not pre-populate these.
...
| `do work run`        | work action     | Start main session loop. Drain the queue.                                 |
...
Agents never perform these operations themselves. The main session triggers cleanup as the final loop step; all other agents leave the archive untouched.
...
| Writing the `## Plan` section into the REQ file        | nestjs-implement (plan-only mode) |
| Writing the `## Plan Hash` section into the REQ file   | nestjs-implement agent |
| Appending `## Plan Verification` to REQ files          | verify-plan action     |
| Moving REQs through `pending -> in-progress -> done`   | main session (per /do-work-run command) |
| Writing `## Override` section on escalation overrides  | main session (per /do-work-run command) |
| Writing `## Loop Counters` section to track cycles     | main session (per /do-work-run command) |
| Moving UR folders to `archive/UR-NNN/`                 | cleanup action         |
| Writing code under `src/` and `test/`                  | nestjs-implement agent |
| TSDoc and markdown edits                               | nestjs-doc-writer agent|
| Scout findings under `do-work/scout/`                  | nestjs-scout agent     |
...
| Writing ADRs under `docs/decisions/`                   | nestjs-doc-writer agent (see knowledge-protocol.md) |
| Writing or bumping proposals under `do-work/proposed-conventions/` | nestjs-module-specialist, nestjs-persistence-specialist (see knowledge-protocol.md) |

**Rules:**

- Agents must not edit `## Verification` or `## Plan Verification` sections. These are skill-owned.
- Agents must not edit the `## Plan Hash` section. It is owned by nestjs-implement and is the drift-detection anchor.
...
- Agents must not move REQ files between states except the main session.
- Agents must not move UR folders between directories except via cleanup.

## Dispatch Brief Budget

Sub-agents run in isolated context windows. That separation is the point. But dispatch briefs that include too much verbatim content defeat the purpose - the isolated context fills up with main-session-injected material before the sub-agent does any work. Long briefs also starve the sub-agent of headroom for its own reasoning and tool output.
...
4. The main session treats it as a standard queue item. The builder reconciles original and addendum at planning time.
