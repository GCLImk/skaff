...
5. Documentation accuracy - do the TSDoc blocks and inline comments
   accurately describe the new behaviour, or are they stale?
...
- Do not chain an external validation onto an external validation. The second reviewer is final. If it fails, the change loops back to `nestjs-implement`, not to a third reviewer.
