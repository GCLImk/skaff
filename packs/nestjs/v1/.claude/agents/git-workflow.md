---
name: git-workflow
description: Manages git operations and GitHub PR lifecycle for NestJS/TypeScript projects using the gh CLI. Use proactively when the user asks to commit, push, open a PR, merge, manage branches, or handle repo config (.gitignore, hooks). Enforces GitHub Flow and Conventional Commits.
tools:
  - Read
  - Glob
  - Grep
...
- Working tree staged or unstaged by `nestjs-implement` and `nestjs-doc-writer`
...
If the repo already uses Husky, lefthook, or simple-git-hooks, add the scan as a new hook entry in the existing config and surface the change for review instead of writing `.git/hooks/pre-commit` directly.
