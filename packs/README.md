...
Current packs include `csharp`, `appsheet`, `python`, `go`, `nestjs`, `nextjs`, `gcli`, `react`, `vue3-vite`, `html-css`, `designer`, `appscript`, `sveltekit`, and `terraform`. The most recent addition is `nestjs`, the first pack whose agent tool allowlists grant `Bash(yarn ...)` rather than `Bash(pnpm ...)`.
