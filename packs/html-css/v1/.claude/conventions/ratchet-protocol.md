...
| --------- | ---------------- |
| `parse_check` | HTML validity and Vite build health. Binary signal. |
| `lint` | ESLint and Stylelint violations relative to changed files. |
| `complexity` | CSS specificity growth and JavaScript complexity across changed modules. |
| `structure` | File layout, naming coherence, token organisation, and project consistency. |
| `dead_code` | Unused CSS selectors, unused JavaScript, and orphaned assets introduced by the change. |
| `accessibility` | WCAG AA conformance, keyboard operation, axe-core, and pa11y results. |
| `doc_quality` | README quality, inline docs where needed, and markdown integrity. |
...
```json
{"req": "REQ-018", "timestamp": "2026-04-24T10:33:00Z", "scope_hint": "broad", "files_changed": 8, "lines_changed": 247, "composite": 0.8652, "dimensions": {"parse_check": 1.0, "lint": 0.82, "complexity": 0.95, "structure": 0.83, "dead_code": 0.98, "accessibility": 0.91, "doc_quality": 0.88}}
```
...
| accessibility | 0.8800 | 0.9150 | +0.035 | Improved |
| doc_quality | 0.8100 | 0.8500 | +0.040 | Improved |
...
If `Kept: No`, the ratchet agent returns a blocking verdict to the main session and the REQ loops back to `html-css-implement` with the failing dimensions as the task brief. The archive step does not run until the ratchet keeps.
...
- **Implicit thresholds**: `parse_check = 1.0`, `accessibility >= 0.85`, `doc_quality >= 0.75`.
