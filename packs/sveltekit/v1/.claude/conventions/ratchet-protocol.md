...
| --------- | ---------------- |
| `parse_check` | TypeScript and Svelte compile health. Binary signal. |
| `lint` | ESLint and project lint violations relative to changed lines and file count. |
| `complexity` | Component, route, load, and action complexity across modified files. |
| `structure` | Project layout, naming, route placement, and server or client boundary consistency. |
| `dead_code` | Unused exports, unreachable branches, and orphaned files introduced by the change. |
| `test_coverage` | Line and branch coverage across modified components, load functions, form actions, stores, and utilities. |
| `doc_quality` | JSDoc or TSDoc completeness on exported APIs plus markdown integrity. |
| `accessibility` | Semantic HTML, labels, keyboard flows, focus handling, and automated axe coverage where available. |

Each dimension is **N/A-aware** - when a dimension genuinely cannot be scored (for example, coverage tooling or accessibility automation is not configured), the score is recorded as `null` and dropped from the composite weighted average rather than penalised as zero.
...
```json
{"req": "REQ-018", "timestamp": "2026-04-24T10:33:00Z", "scope_hint": "broad", "files_changed": 8, "lines_changed": 247, "composite": 0.8742, "dimensions": {"parse_check": 1.0, "lint": 0.84, "complexity": 0.93, "structure": 0.87, "dead_code": 0.98, "test_coverage": 0.76, "doc_quality": 0.89, "accessibility": 0.88}}
```
...
A dimension appears when it was `null` on the previous baseline and has a real value on the current run. This happens when tooling is added mid-project - for example, REQ-003 adds `vitest-axe` and `accessibility` goes from `null` to `0.88`.
...
| accessibility | 0.8800 | 0.9000 | +0.0200 | Improved |
...
```text
Appearance: accessibility = 0.8800 (was null)
```
...
If `Kept: No`, the ratchet agent returns a blocking verdict to the main session and the REQ loops back to `svelte-implement` with the failing dimensions as the task brief. The archive step does not run until the ratchet keeps.
...
- **Implicit thresholds**: `parse_check = 1.0`, `doc_quality >= 0.85`, `test_coverage >= 0.65`, `accessibility >= 0.85`.
