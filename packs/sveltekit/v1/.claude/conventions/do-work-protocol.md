...
├── scout/                          svelte-scout findings briefs
...
| `do-work/scout/` | No | No | svelte-scout only |
...
3. Moves legacy REQs (no `user_request` field) into `archive/legacy/`.
...
| Writing the `## Plan` section into the REQ file | svelte-implement (plan-only mode) |
| Writing the `## Plan Hash` section into the REQ file | svelte-implement agent |
...
| Writing code under `src/` and `tests/` | svelte-implement agent |
| JSDoc, TSDoc, and markdown edits | svelte-doc-writer agent |
| Scout findings under `do-work/scout/` | svelte-scout agent |
...
- Agents must not edit the `## Plan Hash` section. It is owned by svelte-implement and is the drift-detection anchor.
