...
If `Kept: No`, the ratchet agent returns a blocking verdict to the orchestrator and the REQ loops back to `appsheet-implement` with the failing dimensions as the task brief. The archive step does not run until the ratchet keeps.
...
- Do not run the external validator yourself as the implementing agent - the orchestrator dispatches it.
