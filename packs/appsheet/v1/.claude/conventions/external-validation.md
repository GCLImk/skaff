...
A normal review pass (`reviewer` invoked by orchestrator at step 8) uses the reviewer's default brief. This file only applies to the **second, ratchet-triggered pass**.
...
- Do not chain an external validation onto an external validation. The second reviewer is final. If it fails, the change loops back to `appsheet-implement`, not to a third reviewer.
