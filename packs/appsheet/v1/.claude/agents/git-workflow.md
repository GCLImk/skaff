---
name: git-workflow
description: Manages git operations and GitHub PR lifecycle for the ISWG-OS AppSheet governance project using the gh CLI. Use proactively when the user asks to commit, push, open a PR, merge, manage branches, or handle repo config (.gitignore, hooks). Enforces GitHub Flow and Conventional Commits.
tools:
  - Read
  - Glob
  - Grep
...
**Inputs**
- Approved REQ file from orchestrator (verdict: Approve)
- Working tree staged or unstaged by `appsheet-implement` and `appsheet-doc-writer`
...
**Handoff**
- Orchestrator archives the REQ once git operations complete
