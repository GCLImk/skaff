...
| ---------------- | -------------------------------------------------------------------------- |
| `parse_check`    | Changed source files parse without syntax errors. Binary signal.            |
| `lint`           | ESLint and related static-check violations relative to changed files.      |
| `complexity`     | Function length and cyclomatic complexity across modified functions.       |
| `structure`      | Trigger separation, module layout, config boundaries, and quota awareness. |
| `dead_code`      | Unreachable code, unused helpers, orphaned trigger functions, or files.   |
| `test_coverage`  | Line and branch coverage across modified modules via the repo's test path. |
| `doc_quality`    | JSDoc completeness on public functions and markdown integrity.             |
...
If `Kept: No`, the ratchet agent returns a blocking verdict to the main session and the REQ loops back to `appscript-implement` with the failing dimensions as the task brief. The archive step does not run until the ratchet keeps.
