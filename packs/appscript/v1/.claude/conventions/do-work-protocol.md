...
├── scout/                          appscript-scout findings briefs
...
| `do-work/scout/`             | No        | No                  | appscript-scout only         |
...
The main session removes `do-work/.lock` on clean exit (queue drained, escalation paused, or explicit abort). If the process dies uncleanly, the 2-hour staleness threshold eventually releases the lock.
...
| Writing the `## Plan` section into the REQ file        | appscript-implement (plan-only mode) |
| Writing the `## Plan Hash` section into the REQ file   | appscript-implement agent |
...
| Writing code under `src/`, `tests/`, root-level `.gs` / `.js` / `.ts`, and related Apps Script config files | appscript-implement agent |
| JSDoc and markdown edits                               | appscript-doc-writer agent|
| Scout findings under `do-work/scout/`                  | appscript-scout agent     |
...
- Agents must not edit the `## Plan Hash` section. It is owned by appscript-implement and is the drift-detection anchor.
