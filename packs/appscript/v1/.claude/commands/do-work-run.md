---
name: do-work-run
description: Drain the do-work queue. Main session orchestrates; spawns appscript-scout, appscript-implement, appscript-doc-writer, reviewer, ratchet, and git-workflow specialists per REQ. Use when the user says "do work run", "work", "go", "start", or confirms the work prompt.
allowed-tools:
  - Read
  - Write
  - Edit
...
| -------------------------------- | ---------------------------- |
| `## Plan` or `## Plan Hash` content | `appscript-implement` in plan-only mode |
| Any file under `src/`, `tests/`, or root-level `.gs` / `.js` / `.ts` / `appsscript.json` / `.clasp.json` / `.claspignore` / `package.json` / `tsconfig.json` | `appscript-implement` in implement mode |
| JSDoc edits, README updates, doc audit tables | `appscript-doc-writer` |
...
| Scout findings briefs | `appscript-scout` |
...
1. **Clarification** - the REQ body, scout findings, or a specialist's output contains an Open Question, a "resolve before implementation" note, or a decision that has no defensible default. Examples: a missing script property name, a sheet name not in the existing schema, or a trigger event shape the project has not established.
2. **Architectural decisions** - a plan or implementation proposes a cross-REQ or cross-surface change not explicitly authorised in the REQ (new trigger family, new external integration, new dependency, new runtime or tooling baseline). Do not let a specialist silently expand scope; surface it.
...
4. **Scout** (medium and complex only): spawn `appscript-scout` via `Agent(subagent_type: "appscript-scout", prompt: <dispatch brief>)`. Wait for its findings brief at `do-work/scout/REQ-NNN-*-findings.md`.
5. **Plan**: spawn `appscript-implement` in **plan-only mode**. Brief must name the mode explicitly. The agent writes `## Plan` and `## Plan Hash` sections inline in the REQ file and returns without touching code. Plan depth scales to complexity (Simple: 1-3 bullets; Medium: ordered steps with verification checks; Complex: ordered steps, dependencies, testing approach, rollback notes).
...
7. **Implement**: spawn `appscript-implement` with the full REQ content (including `## Plan`) in **implement mode**.
8. **Documentation**: if complex, or if docs are explicitly in scope, spawn `appscript-doc-writer`.
9. **Review**: spawn `reviewer`. Act on verdict:
   - `Approve` - proceed to step 10.
   - `Request Changes` - increment the REQ's `implement_review_ratchet_cycles` counter (Loop Counters section), write back. If counter > 5, escalate. Otherwise re-spawn `appscript-implement` with blocking issues as the task brief. Return to step 9.
...
10. **Ratchet**: spawn `ratchet`. Act on verdict:
    ...
    - `Rejected` - increment the loop counter. If > 5, escalate. Otherwise re-spawn `appscript-implement` with failing dimensions from Blocking Dimensions. Return to step 9 (reviewer re-reviews before ratchet re-runs).
...
After the queue is drained, the `cleanup` skill action runs automatically. Do not perform it manually. Then release the lock.
...
- Same blocking issue returned to `appscript-implement` more than twice.
...
2. **Split REQ into smaller units** - move to `do-work/archive/abandoned/REQ-NNN-abandoned.md` with a note. Continue with the next pending REQ.
...
Counters survive process restart. On re-spawn, read the current value, increment, write it back, and pass the updated count in the dispatch brief.
...
- [ ] All pending REQ files either archived to `do-work/archive/` or paused with a documented escalation reason
