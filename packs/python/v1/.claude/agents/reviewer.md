...
**Handoff**
- Main session routes on verdict: Approve → ratchet (then git-workflow if Kept), Request Changes → python-implement, Escalate → user
...
2. Read in this order: `do-work/working/REQ-*.md` (including its inline `## Plan` and `## Plan Verification` sections), `git diff HEAD`, full contents of every changed file, `python -m py_compile` output on changed files, `ruff check` output, `pytest` output, and `coverage` output if available.
3. Check: requirement coverage against the REQ file, regression risk, architecture fit, test sufficiency, docstring coverage on public symbols (via `interrogate` if configured), and conventions compliance.
