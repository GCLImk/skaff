---
name: do-work-run
description: Drain the do-work queue. Main session orchestrates; spawns python-scout, python-implement, python-doc-writer, reviewer, ratchet, and git-workflow specialists per REQ. Use when the user says "do work run", "work", "go", "start", or confirms the work prompt.
allowed-tools:
  - Read
  - Write
  - Edit
...
| --------------------------------------------------------- | ---------------------------------------------- |
| `## Plan` or `## Plan Hash` content                       | `python-implement` in plan-only mode         |
| Any `.cs` file under `src/` or `tests/`                   | `python-implement` in implement mode           |
| JSDoc edits, README updates, doc audit tables             | `python-doc-writer`                          |
...
| Scout findings briefs                                     | `python-scout`                               |
...
4. **Scout** (medium and complex only): spawn `python-scout` via `Agent(subagent_type: "python-scout", prompt: <dispatch brief>)`. Wait for its findings brief at `do-work/scout/REQ-NNN-*-findings.md`.
5. **Plan**: spawn `python-implement` in **plan-only mode**. Brief must name the mode explicitly. The agent writes `## Plan` and `## Plan Hash` sections inline in the REQ file and returns without touching code. Plan depth scales to complexity (Simple: 1-3 bullets; Medium: ordered steps with verification checks; Complex: ordered steps, dependencies, testing approach, rollback notes).
...
7. **Implement**: spawn `python-implement` with the full REQ content (including `## Plan`) in **implement mode**.
8. **Documentation**: if complex, or if docs are explicitly in scope, spawn `python-doc-writer`.
9. **Review**: spawn `reviewer`. Act on verdict:
   - `Approve` - proceed to step 10.
   - `Request Changes` - increment the REQ's `implement_review_ratchet_cycles` counter (Loop Counters section), write back. If counter > 5, escalate. Otherwise re-spawn `python-implement` with blocking issues as the task brief. Return to step 9.
...
10. **Ratchet**: spawn `ratchet`. Act on verdict:
    ...
    - `Rejected` - increment the loop counter. If > 5, escalate. Otherwise re-spawn `python-implement` with failing dimensions from Blocking Dimensions. Return to step 9 (reviewer re-reviews before ratchet re-runs).
...
- Same blocking issue returned to `python-implement` more than twice.
