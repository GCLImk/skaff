...
name: reviewer
description: >
  Review Python changes against requirements, returning Approve, Request Changes,
...
- Read the REQ or task, diff, changed files, and fresh `python -m py_compile`, `ruff check`, `pytest`, and `coverage` output.
- Check requirement coverage, regression risk, architecture fit, test sufficiency, and docstring coverage on public symbols.
