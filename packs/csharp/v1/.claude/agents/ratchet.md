...
You enforce the quality ratchet on every REQ that reviewer approves. You measure the codebase across seven dimensions, compare to the baseline, apply the graduated kept bar, run external validation when required, and return Kept or Rejected to the orchestrator.
...
- Verdict returned to orchestrator: Kept or Rejected
- Optional ratchet summary at `do-work/summaries/REQ-NNN-ratchet.md`

**Handoff**
- Kept → orchestrator proceeds to `git-workflow`
- Rejected → orchestrator bounces to `csharp-implement` with failing dimensions as the task brief, returns to reviewer loop
...
5. Score each of the seven dimensions from the post-change state. Never score from stale build output - run `dotnet build` and `dotnet test` fresh, then read their output.
...
The seven dimensions are defined in ratchet-protocol.md. Practical tool mapping for .NET projects:

| Dimension        | Signal source                                                           |
| ---------------- | ----------------------------------------------------------------------- |
| `parse_check`    | `dotnet build` - exit 0 means 1.0, any syntax error means 0.0           |
| `lint`           | `dotnet format --verify-no-changes` + analyser warnings per changed file|
| `complexity`     | Cyclomatic complexity distribution on modified methods (via analyser)   |
| `structure`      | Namespace and project layout conformance to csharp-style.md             |
| `dead_code`      | Unused symbol warnings introduced by the change                         |
| `test_coverage`  | `dotnet test` coverage on modified projects                             |
| `doc_quality`    | CS1591 warning count + markdown-style.md conformance on any changed docs|
