...
Drain the do-work work queue, processing pending REQs through the skaff pipeline.
...
4. For each pending REQ:
   ...
   d. Write a `## Plan` section into the REQ (implement agent)
   e. Implement the code changes per the plan
   f. Run `dotnet build` and `dotnet test` - fix failures
   g. Review the diff for correctness, tests, and style
