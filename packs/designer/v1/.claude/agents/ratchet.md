...
- Rejected → main session bounces to `designer-implement` with failing dimensions as the task brief, returns to reviewer loop
...
5. Score each of the seven dimensions from the post-change state. Never score from stale validation output - run `npm run build`, `npm run lint`, `npx stylelint`, and a Storybook build fresh, then read their output together with configured Chromatic and axe-core results.
...
| --------- | ------------- |
| `parse_check` | `npm run build` plus Storybook build - exit 0 means 1.0, parse or bundle failure means 0.0 |
| `lint` | `npm run lint`, `npx stylelint`, and any component-lint output on changed files |
| `complexity` | Variant explosion, selector nesting, and component or story complexity on changed files |
| `structure` | Token layout, component organisation, story alignment, and naming hygiene |
| `dead_code` | Unused tokens, dead component branches, and orphaned stories introduced by the change |
| `accessibility` | axe-core, Storybook a11y checks, focus states, keyboard flows, and contrast quality |
| `doc_quality` | Storybook stories, README quality, component docs, and markdown-style.md conformance on changed docs |
