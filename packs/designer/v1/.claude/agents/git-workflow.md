---
name: git-workflow
description: Manages git operations and GitHub PR lifecycle for design-system projects using the gh CLI. Use proactively when the user asks to commit, push, open a PR, merge, manage branches, or handle repo config (.gitignore, hooks). Enforces GitHub Flow and Conventional Commits.
tools:
  - Read
  - Glob
  - Grep
