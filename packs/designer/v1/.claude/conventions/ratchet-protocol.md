...
| --------- | ---------------- |
| `parse_check` | CSS or SCSS parsing plus build and Storybook build health. Binary signal. |
| `lint` | Stylelint and ESLint violations relative to changed files. |
| `complexity` | Design-system coherence, selector nesting, and component complexity growth. |
| `structure` | Token layout, component organisation, story coverage alignment, and naming hygiene. |
| `dead_code` | Unused tokens, dead component branches, and orphaned stories introduced by the change. |
| `accessibility` | WCAG AA conformance, focus states, contrast, and axe-core results. |
| `doc_quality` | Storybook stories, README quality, component docs, and markdown integrity. |
...
accessibility   2.0    # defining quality gate
doc_quality     2.0    # Storybook deliverables matter
```

`parse_check` is a gate, not a weight. If `parse_check < 1.0`, the composite is 0 regardless of other scores. No other signal can rescue a change that does not parse or build.
...
```json
{"req": "REQ-018", "timestamp": "2026-04-24T10:33:00Z", "scope_hint": "broad", "files_changed": 8, "lines_changed": 247, "composite": 0.8652, "dimensions": {"parse_check": 1.0, "lint": 0.82, "complexity": 0.94, "structure": 0.86, "dead_code": 0.97, "accessibility": 0.90, "doc_quality": 0.92}}
```
...
The bar also **scales with change scope**. A focused fix should not need three dimensions improving to pass. Scope is objectively derived, not agent-claimed.
...
| 0.70 - 0.84       | Composite does not decrease. No dimension regresses beyond threshold. | Composite does not decrease. Two dimensions improve. No dimension regresses by > 0.02. |
| 0.85 - 0.92       | Composite does not decrease. At least one dimension improves. External validation passes. | Composite increases by >= 0.005. Three dimensions improve. Zero regressions. External validation passes. |
| 0.93 and above    | Composite does not decrease. At least one dimension improves. External validation passes. Override required in REQ. | Composite increases by >= 0.005. Three dimensions improve. Zero regressions. External validation passes. Override required in REQ. |
...
**Kept criterion**: 0.85-0.92 / broad - requires +0.005 composite, 3 dimensions improving, 0 regressions, external validation
**Kept**: Yes
...
| complexity | 0.9400 | 0.9500 | +0.010 | Improved |
| structure | 0.8600 | 0.8850 | +0.025 | Improved |
| dead_code | 0.9700 | 0.9700 | 0.0000 | Held |
| accessibility | 0.8900 | 0.9150 | +0.025 | Improved |
| doc_quality | 0.9000 | 0.9400 | +0.040 | Improved |
...
```text
Appearance: accessibility = 0.9100 (was null)
```
...
If `Kept: No`, the ratchet agent returns a blocking verdict to the main session and the REQ loops back to `designer-implement` with the failing dimensions as the task brief. The archive step does not run until the ratchet keeps.
...
- **Implicit thresholds**: `parse_check = 1.0`, `accessibility >= 0.85`, `doc_quality >= 0.85`.
