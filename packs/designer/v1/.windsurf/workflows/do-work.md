...
4. For each pending REQ:
   ...
   c. For medium/complex: scout the design system before planning
   d. Write a `## Plan` section into the REQ (`designer-implement` in plan-only mode)
   e. Implement the token, component, theming, and story changes per the plan
   f. Run `npm run build`, `npm run lint`, `npx stylelint`, and a Storybook build - fix failures
   g. Review the diff for correctness, accessibility, and story coverage
