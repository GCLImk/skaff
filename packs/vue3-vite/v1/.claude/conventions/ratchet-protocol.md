...
| `complexity` | Component and composable size, nested template logic, and reactive effect complexity across modified files. |
| `structure` | Project layout, naming, component placement, and composable or store usage consistency. |
| `dead_code` | Unused exports, unreachable branches, and orphaned files introduced by the change. |
| `test_coverage` | Line and branch coverage across modified components, composables, stores, and utilities. |
...
If `Kept: No`, the ratchet agent returns a blocking verdict to the main session and the REQ loops back to `vue3-implement` with the failing dimensions as the task brief. The archive step does not run until the ratchet keeps.
...
- **Implicit thresholds**: `parse_check = 1.0`, `doc_quality >= 0.85`, `test_coverage >= 0.70`, `accessibility >= 0.85`.
